...
<form method="post" enctype="multipart/form-data"
      action="<?= url($item ? '/admin/blogs/update/' . $item['id'] : '/admin/blogs/store') ?>"
  ...
  <div class="grid sm:grid-cols-2 gap-5">
    ...
    <?php $f(['name' => 'author', 'label' => 'Author', 'value' => old_raw('author', $item['author'] ?? 'Pakistan Cambridge School')]); ?>
    ...
    <div class="sm:col-span-2">
      <?php $f(['name' => 'image', 'label' => 'Featured Image (optional)', 'type' => 'file', 'help' => 'Shown on the blog list and article page. Recommended 1200×675.']); ?>
  ...
  <div class="mt-7 flex gap-3">
    ...
    <a href="<?= url('/admin/blogs') ?>" class="inline-flex items-center h-11 px-6 rounded-lg text-ink-700 font-medium hover:bg-sunken transition">Cancel</a>
  </div>
</form>
<?php clear_old(); ?>
